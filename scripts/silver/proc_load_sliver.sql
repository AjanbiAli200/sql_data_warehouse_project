	...
	BEGIN TRY
		...
		SELECT
			...
			CASE WHEN sls_price IS NULL OR sls_price <= 0
				...
				ELSE sls_price
			END AS sls_price				--Derive pice if original value is invalid
		...
		SELECT
			...
			CASE WHEN bdate > GETDATE() THEN NULL
				 ELSE bdate
			END bdate,				--Set future birthdaydates to NULL
		...
		SELECT
			REPLACE(cid, '-', '') AS cid,			--Handled in valid values
			CASE WHEN TRIM(cntry) = 'DE' THEN 'Germany'
				 WHEN TRIM(cntry) IN ('US', 'USA') THEN 'United States'
				 WHEN TRIM(cntry) = '' OR cntry IS NULL THEN 'n/a'
				 ELSE TRIM(cntry)
			END AS cntry				--Data Standardization & Consistency
